...
struct Scanner[R: io.Reader]():
        ...
        while True:
            ...
            if (self.end > self.start) or self.err:
                ...
                if self.err:
                    at_eof = True
                advance = self.split(
                    self.buf[self.start : self.end], at_eof, token, err
                )
                if err:
                    if str(err.value()) == ERR_FINAL_TOKEN:
                        self.token = token
                        self.done = True
    ...
    fn set_err(inout self, err: Err):
        ...
        """
        if self.err:
            var value = String(self.err.value())
...
# TODO: For now, passing in token and err to be modified by the SplitFunction. This is because the Tuple return unpacking cannot handle a memory only type (List[Byte] and Err)
alias SplitFunction = fn (
    data: List[Byte], at_eof: Bool, inout token: List[Byte], inout err: Err
) raises -> Int
...
fn drop_carriage_return(data: List[Byte]) raises -> List[Byte]:
...
fn scan_lines(
    ...
    token = drop_carriage_return(data[0:i])
    if i >= 0:
